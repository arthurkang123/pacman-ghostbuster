...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for posDist in livingGhostPositionDistributions:
            temp = self.distancer.getDistance(pacmanPosition, posDist.argMax())
            if first == 1 or temp <= distance:
                first = 1
                distance = temp
                gPos = posDist.argMax()
        ...
        first = 1
        for l in legal:
            temp = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, l), gPos)
            if first == 1 or temp <= distance:
                first = 0
                distance = temp
                result = l
